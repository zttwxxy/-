...
edges_x = cv2.filter2D(blurred, cv2.CV_16S, kernel_roberts_x)
edges_y = cv2.filter2D(blurred, cv2.CV_16S, kernel_roberts_y)
...
cv2.imshow("Detected Cars with Robert", image)
cv2.waitKey(0)
